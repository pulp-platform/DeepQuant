...
def prepareFQResNet50():
    """Prepare a fake-quantized (FQ) ResNet50 model."""
...
def deepQuantTestResnet50() -> None:
    HOME = Path.home()
    ...
    originalModel = torchvision.models.resnet50(
        weights=torchvision.models.ResNet50_Weights.IMAGENET1K_V2
    )
    originalModel = originalModel.eval().to(device)
    print("Original ResNet50 loaded.")

    print("Evaluating original model...")
    originalTop1, originalTop5 = evaluateModel(
        originalModel, valLoader, device, "Original ResNet50"
    )

    print("Preparing and quantizing ResNet50...")
    FQModel = prepareFQResNet50()
    ...
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    FQTop1, FQTop5 = evaluateModel(FQModel, valLoader, device, "FQ ResNet50")
    ...
    TQTop1, TQTop5 = evaluateModel(TQModel, valLoader, device, "TQ ResNet50")

    print("\nComparison Summary:")
    print(f"{'Model':<25} {'Top-1 Accuracy':<25} {'Top-5 Accuracy':<25}")
    print("-" * 75)
    print(f"{'Original ResNet50':<25} {originalTop1:<24.2f} {originalTop5:<24.2f}")
    print(f"{'FQ ResNet50':<25} {FQTop1:<24.2f} {FQTop5:<24.2f}")
    print(f"{'TQ ResNet50':<25} {TQTop1:<24.2f} {TQTop5:<24.2f}")
    print(
